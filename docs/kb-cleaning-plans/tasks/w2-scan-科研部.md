...
## 工作目录
`C:\Users\Administrator\Documents\code\kb-pipeline\ws2-website-scrape\scraped-pages\科研部`

## 任务概述
扫描科研部目录下的所有 .md 文件（共 38 个），按照知识库入库标准进行筛选和分类，产出结构化扫描报告。

## 输入
- 待扫描目录：`C:\Users\Administrator\Documents\code\kb-pipeline\ws2-website-scrape\scraped-pages\科研部`
...
5. 对保留的文件标注分类（该部门主要对应 C10 学术与竞赛）
6. 按照 SCAN-REPORT-FORMAT.md 的格式生成报告

## 输出
将完整的扫描报告写入：
`C:\Users\Administrator\Documents\code\kb-pipeline\ws2-website-scrape\kimi-reports\scan-科研部.md`
